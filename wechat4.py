...
def get_tuling_reponse(msg0):
    ...
    apikey_3 = '********************************'
    apikey_4 = '********************************'
    apikey_5 = '********************************'#图灵机器人apikey，自己注册账号可获取
    apikeys = [apikey_1,apikey_2,apikey_3,apikey_4,apikey_5]
    ...
    for i in range(5):
        ...
        fileObject_robot = open('robot.txt','a+')
        fileObject_robot.write('TL:'+res['results'][0]['values']['text'])
        fileObject_robot.write('\n')
        fileObject_robot.close()
        fileObject_people = open('people.txt','a+')
...
def draw_wordcloud(file):
    #读入一个txt文件
    try:
        comment_text = open(file,'r',encoding='utf-8').read()
    except:
...
def simple_reply(msg):
  ...
  if msg['Content']=='群发1010110':
      ...
      sta3='robot'
      return '图灵/百度ai机器人启动成功'
  elif msg['Content']=='closerobot':
      sta3=''
      return '图灵/百度ai机器人关闭成功'
  elif sta3=='robot':
